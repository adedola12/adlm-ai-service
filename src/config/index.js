...
export const config = {
  ...
  featureWeights: {
    ...
    billCleanup: 2,
    billAsk: 1,
    // Batches across a whole bill and can escalate a batch, like billCleanup.
    breakdownFill: 2,
    // Batches across a whole client bill and can escalate a batch.
    boqFill: 2,
  },
  ...
  // breakdownFill is an enrichment of a budget that already works without it
  // (the uncovered remainder shows as one balancing line either way), so it can
  // wait when credit is tight.
  nonCriticalFeatures: ["outliers", "catalogueExtract", "billAsk", "breakdownFill"],
  ...
  // 09-23.7 boq-fill shortlist: a candidate must share a word with the ITEM, not
  //     just with its section — "Services" was reaching every services line.
  // 09-19.6 boq-fill: stage/sheet decides the element of a line that names none,
  //     bar diameters are a hard qualifier (results are cached post-rule).
  // 09-19.5 boq-fill on four live house styles: sheet name leads the section,
  //     one element per line (enforced in code too), DPM/DPC/BRC/Y10 expansions.
  // .4  boq-fill added: a client's own bill filled from measured quantities;
  //     the model picks measured-line groups, the service sums and converts.
  // .3  breakdown-fill reads a bill the way a QS does: the item's heading trail
  //     and its "Ditto"/lower-case continuation are now part of the prompt.
  //     Measured against five live Nigerian bills, a description on its own is
  //     a fragment ("Skirting", "Column bases; thickness not exceeding 50mm"),
  //     so answers cached under .2 were breaking down the wrong work.
  // .2  breakdown-fill added: material/labour content of a BoQ item worked out
  //     from its description, per one unit, priced from the master lists.
  promptVersion: "2026-09-23.9",
